 ...
 * 
 * CRITICAL: Never use supabase.from() directly in components or libraries.
...
export class SecureAPIClient {
  ...
  /**
   * ✅ FIX: Get user session identifier for better request deduplication isolation
  ...
  /**
   * Emergency security method to clear all cache and session data
   */
  ...
  private async generateCacheKey(method: string, endpoint: string, tenantId: string): Promise<string> {
    // ✅ FIX: Include user session key to prevent cross-user request conflicts
  ...
  private async request<T>(
    ...
    if (isGetRequest) {
      ...
      if (pending) {
        ...
        try {
          // ✅ FIX: Add timeout to prevent hanging on stuck pending requests
          ...
          // ✅ FIX: Clean up failed pending request
